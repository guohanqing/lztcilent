...
static struct rt_uart_extent_device _finger_dev;
...
static void usart_fp_data_thread_entry(void *parameter)
{
    ...
    rt_uint8_t fp_recv_buf[512] = {0};
    while (1)
    {
			//struct rt_mailbox fp_recv_mail;
			#if 1
   
      result = rt_event_recv(&fp_recv_event,REV_DATA,RT_EVENT_FLAG_OR | RT_EVENT_FLAG_CLEAR,10*RT_TICK_PER_SECOND,&event);
      ...
      { 
            ...
            {
					#if 1
					if((0x23 == temp_uart2_recv[1])&&(0xf5 == temp_uart2_recv[507])&&(ACK_SUCCESS == temp_uart2_recv[4]))
					{
						rt_memcpy(fp_recv_buf+2,temp_uart2_recv,508);
						
						temp_uart_i = 0;
						//rt_mb_send(&fp_mail,(rt_uint32_t)(fp_recv_buf+12));
                        fp_recv_buf[0] =  ID_TASK_SEND_FP;
                        fp_recv_buf[1] = RCV_DEVICE;
                        rt_mq_send(&wmq,fp_recv_buf,512);
                        //_finger_dev.parent.control(&_finger_dev.parent,FP_REG,RT_NULL);
                        ...
                        //ta0702_fp_close(&_finger_dev.parent);
					...
					else if((0x23 == temp_uart2_recv[1])&&(ACK_SUCCESS != temp_uart2_recv[4]))
					{
                        fp_recv_buf[0] =  0;
                        fp_recv_buf[1] = RCV_DEVICE;
                        fp_recv_buf[2] = RT_FALSE;
						//rt_memcpy(temp_uart2_recv,0,508);
                        //rt_memcpy(fp_recv_buf+2,temp_uart2_recv,507);
                        rt_mq_send(&wmq,fp_recv_buf,3);
						rt_memset(temp_uart2_recv,0,508);
						temp_uart_i = 0;
                        //_finger_dev.parent.control(&_finger_dev.parent,FP_REG,RT_NULL);
						rt_thread_delete(finger_thread);
                    ...
                    else if((0x23 == temp_uart2_recv[1])&&(ACK_TIMEOUT == temp_uart2_recv[4]))
					{
                        fp_recv_buf[0] =  ID_TASK_SEND_FP;
                        fp_recv_buf[1] = RCV_DEVICE;
                        fp_recv_buf[2] = ACK_TIMEOUT;
                        rt_mq_send(&wmq,fp_recv_buf,3);         
                        rt_memset(temp_uart2_recv,0,508);
                        temp_uart_i = 0;
                        rt_event_detach(&fp_recv_event);
			...
			case FP_REG:
				 {
                     ...
                     finger_thread = rt_thread_find("fingerprint");
